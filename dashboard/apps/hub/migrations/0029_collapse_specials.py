...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # Group special elections by state, end_date and race_type
        groups = {}
        for elec in orm['hub.Election'].objects.filter(special=True):
            key = (elec.state_id, elec.end_date, elec.race_type)
            groups.setdefault(key, []).append(elec)

        # Collapse multiple specials on same date into a single record,
